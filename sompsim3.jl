...
function sompsim(stim)
  ...
  projections=readdlm("projections3.txt")
  J=readdlm("J3.txt")
  Ks=readdlm("Ks3.txt")
  tau=readdlm("tau3.txt")
  Nns=readdlm("Nns3short.txt")
  Ne=convert(Int64,Nns[1])
  Np=convert(Int64,Nns[2])
  Ns=convert(Int64,Nns[3])
  N0=convert(Int64,Nns[4])
  Ni=Np+Ns
  Ncells=Ne+Ni+N0
...
# 	gL[(1+Ne):(Ne+Ni)] = gIL
  ...
  adaptInput = zeros(Ne+Ni)
	...
	#v1=zeros(Nsteps,Ne+Ni)
	#v1[1,:]=v
	#fIE1=zeros(Nsteps,Ne+Ni)
	#fII1=zeros(Nsteps,Ne+Ni)
	#fI01=zeros(Nsteps,Ne+Ni)
	#aI1=zeros(Nsteps,Ne+Ni)
	println("starting simulation")

	#begin main simulation loop
	for ti = 1:Nsteps
		#v1[ti,:]=v
		#fIE1[ti,:]=forwardInputsE
		#fII1[ti,:]=forwardInputsI
		#fI01[ti,:]=forwardInputs0
		#aI1[ti,:]=adaptInput
